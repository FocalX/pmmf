			...
			if(!$request->getError()) {
				...
				if(!empty($user_info)) {
					if($user_info['status'] == usersModel::USER_STATUS_ACTIVE) { // this checks for disabled user
						$access_info = apiHelpers::setAccess($this->access_control, $user_info['id'], $user_info['type'], auth::$area_api);
						if($access_info !== FALSE) {
							$request->setJsonReturnData($access_info);
							
						} else {
							throw new pmmfException('Authentication error', 401,
									array($logging::LOG_LEVEL_WARN, "Error set up authentication session  (login) (user_id=$user_info[id]))"));
						}
					} else {
						throw new pmmfException('Account not active', 403,
								array($logging::LOG_LEVEL_ERROR, "Non-active user account login attempt (by password) (user_id=$user_id / email=$email / handle=$handle)"));
					}
				} else {
					throw new pmmfException('Invalid credential', 401,
							array($logging::LOG_LEVEL_WARN, "Wrong user identifier and token/password combination (login) (user_id=$user_id / email=$email / handle=$handle / facebook_id=$facebook_id)"));
				}
			}
		}
	}
	...
	function refreshSession() {
		...
		if(!$request->getError()) {
			...
			if(!$request->getError()) {
				...
				if(!empty($user_info)) {
					if($user_info['status'] == usersModel::USER_STATUS_ACTIVE) { // this checks for disabled user
						$access_info = apiHelpers::refreshAccess($this->access_control, $user_id, auth::$area_api, $refresh_token);
